...
This document provides guidelines for maintaining the `openai-agents-skills` package, including the release process, CI/CD workflows, and ongoing maintenance responsibilities.
...
3. Update version in `pyproject.toml` and `_version.py`
...
|-------------|--------------|---------|
| `feat:` | Minor (0.1.0 → 0.2.0) | `feat: add skill lifecycle hooks` |
| `fix:` | Patch (0.1.0 → 0.1.1) | `fix: resolve tool merge ordering` |
| `perf:` | Patch | `perf: optimise registry lookup` |
| `BREAKING CHANGE:` | Major (0.1.0 → 1.0.0) | `feat!: redesign SkillRegistry API` |
...
- Verify the package on PyPI: https://pypi.org/project/openai-agents-skills/
- Test installation: `pip install openai-agents-skills==X.Y.Z`
...
1. **Create a PyPI API token:**
   ...
   - Create a token scoped to the `openai-agents-skills` project (or account-wide for first publish)
...
3. Update the package code to maintain compatibility
...
|---------|---------|------------------|
| `openai-agents` | SDK being extended | Monitor weekly |
