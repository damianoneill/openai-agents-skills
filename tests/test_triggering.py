"""Tests for skill triggering behaviour.
...
class TestSkillPhase4Attributes:
    ...
    def test_triggers_after_tools_instance_list_does_not_share_class_default(self) -> None:
        ...
        sk_a = SimpleSkill("a", triggers_after_tools=["t1"])
        sk_b = SimpleSkill("b")
...
class TestRegistryGetTriggeredByTool:
    def test_returns_skill_whose_triggers_after_tools_matches(self) -> None:
        ...
        skill = SimpleSkill("post-write", triggers_after_tools=["write_file"])
    ...
    def test_returns_empty_list_when_no_skills_match(self) -> None:
        registry = SkillRegistry()
        registry.register(SimpleSkill("other"))
    ...
    def test_only_returns_skills_matching_the_tool_name(self) -> None:
        registry = SkillRegistry()
        registry.register(SimpleSkill("a", triggers_after_tools=["write_file"]))
        registry.register(SimpleSkill("b", triggers_after_tools=["read_file"]))
    ...
    def test_skill_triggerable_by_multiple_tools(self) -> None:
        registry = SkillRegistry()
        skill = SimpleSkill("multi", triggers_after_tools=["tool_a", "tool_b"])
        registry.register(skill)
    ...
    def test_disabled_skill_not_returned(self) -> None:
        registry = SkillRegistry()
        registry.register(SimpleSkill("off", triggers_after_tools=["t"], enabled=False))
        assert registry.get_triggered_by_tool("t") == []

    def test_multiple_skills_triggered_by_same_tool(self) -> None:
        registry = SkillRegistry()
        skill_a = SimpleSkill("a", triggers_after_tools=["deploy"])
        skill_b = SimpleSkill("b", triggers_after_tools=["deploy"])
...
class TestRegistryGetPostTurn:
    ...
    def test_disabled_post_turn_skill_excluded(self) -> None:
        registry = SkillRegistry()
        registry.register(SimpleSkill("off", triggers_after_turn=True, enabled=False))
        assert registry.get_post_turn() == []

    def test_mixed_registry_only_returns_post_turn_flagged(self) -> None:
        registry = SkillRegistry()
        always = SimpleSkill("always")
        post = SimpleSkill("post", triggers_after_turn=True)
...
class TestToolResultTriggersSkillHooks:
    async def test_matching_tool_queues_skill_in_pending(self) -> None:
        ...
        await hooks.on_tool_end(
            make_mock_context(), make_mock_agent(), make_mock_tool("write_file"), "ok"
        )
    ...
    async def test_non_matching_tool_does_not_queue_skill(self) -> None:
        registry = SkillRegistry()
        registry.register(SimpleSkill("post-write", triggers_after_tools=["write_file"]))
        hooks = make_hooks(registry)

        await hooks.on_start(make_mock_context(), make_mock_agent())
        await hooks.on_tool_end(
            make_mock_context(), make_mock_agent(), make_mock_tool("read_file"), "ok"
        )

        assert _get_run_state().pending_skills == []

    async def test_no_registry_does_nothing(self) -> None:
        hooks = SkillHooks([SimpleSkill("x")])
        await hooks.on_start(make_mock_context(), make_mock_agent())
        await hooks.on_tool_end(make_mock_context(), make_mock_agent(), make_mock_tool("any"), "ok")
        assert _get_run_state().pending_skills == []
    ...
    async def test_same_skill_not_queued_twice_for_same_tool(self) -> None:
        ...
        registry = SkillRegistry()
        skill = SimpleSkill("s", triggers_after_tools=["t"])
        registry.register(skill)
        hooks = make_hooks(registry)

        await hooks.on_start(make_mock_context(), make_mock_agent())
        await hooks.on_tool_end(make_mock_context(), make_mock_agent(), make_mock_tool("t"), "ok")
        await hooks.on_tool_end(make_mock_context(), make_mock_agent(), make_mock_tool("t"), "ok")
...
class TestPendingDrain:
    async def test_pending_skill_injected_at_next_llm_start(self) -> None:
        registry = SkillRegistry()
        skill = SimpleSkill("triggered", triggers_after_tools=["t"], content="triggered!")
        registry.register(skill)
        hooks = make_hooks(registry)
    ...
    async def test_pending_queue_cleared_after_drain(self) -> None:
        registry = SkillRegistry()
        registry.register(SimpleSkill("s", triggers_after_tools=["t"]))
        hooks = make_hooks(registry)
    ...
    async def test_pending_not_reinjected_on_subsequent_llm_start(self) -> None:
        ...
        registry.register(SimpleSkill("s", triggers_after_tools=["t"], content="ONCE"))
        hooks = make_hooks(registry)
        ...
        await hooks.on_llm_start(make_mock_context(), make_mock_agent(), None, items1)

        # Second call — pending is empty; skill must NOT re-appear
        await hooks.on_llm_end(make_mock_context(), make_mock_agent(), make_mock_response())
        items2: list[Any] = [{"role": "user", "content": "y"}]
        await hooks.on_llm_start(make_mock_context(), make_mock_agent(), None, items2)
    ...
    async def test_multiple_pending_skills_all_injected(self) -> None:
        registry = SkillRegistry()
        skill_a = SimpleSkill("a", triggers_after_tools=["t"], content="block_a")
        skill_b = SimpleSkill("b", triggers_after_turn=True, content="block_b")
        registry.register(skill_a)
        registry.register(skill_b)
        hooks = make_hooks(registry)
    ...
    async def test_disabled_pending_skill_not_injected(self) -> None:
        ...
        registry = SkillRegistry()
        skill = SimpleSkill("s", triggers_after_tools=["t"], content="should-not-appear")
        registry.register(skill)
        hooks = make_hooks(registry)
...
class TestDrainPending:
    ...
    async def test_drains_and_returns_blocks(self) -> None:
        skill = SimpleSkill("s", content="drained!")
        state = RunState()
    ...
    async def test_pending_cleared_after_drain(self) -> None:
        state = RunState()
        state.pending_skills.append(SimpleSkill("s"))
    ...
    async def test_already_injected_skill_skipped(self) -> None:
        ...
        skill = SimpleSkill("s", content="should-not-appear")
        state = RunState()
        state.pending_skills.append(skill)
...
class TestCrossAgentDrain:
    async def test_tool_trigger_on_agent_a_drains_into_agent_b_llm_start(self) -> None:
        ...
        await hooks.on_agent_start(make_mock_context(), agent_a)
        await hooks.on_tool_end(make_mock_context(), agent_a, make_mock_tool("deploy"), "ok")
        await hooks.on_llm_end(make_mock_context(), agent_a, make_mock_response())

        # Agent B's on_llm_start should drain the pending skill
        input_items: list[Any] = [{"role": "user", "content": "continue"}]
        await hooks.on_llm_start(make_mock_context(), agent_b, None, input_items)
...
class TestRunSkillHooksTriggers:
    ...
    async def test_non_matching_tool_does_not_queue(self) -> None:
        registry = SkillRegistry()
        registry.register(SimpleSkill("s", triggers_after_tools=["write_file"]))
        hooks = make_run_hooks(registry)
    ...
    async def test_no_registry_on_tool_end_does_nothing(self) -> None:
        hooks = RunSkillHooks([SimpleSkill("x")])
        await hooks.on_agent_start(make_mock_context(), make_mock_agent())
        await hooks.on_tool_end(make_mock_context(), make_mock_agent(), make_mock_tool("any"), "ok")
        assert _get_run_state().pending_skills == []
...
class TestHookDeduplication:
    async def test_tool_trigger_not_double_queued_when_both_hooks_active(self) -> None:
        ...
        sh = make_hooks(registry)
        rh = make_run_hooks(registry)

        # Initialise RunState via both hooks
        await sh.on_start(make_mock_context(), make_mock_agent())
        await rh.on_agent_start(make_mock_context(), make_mock_agent())

        # Both hooks fire on_tool_end for the same tool
        await sh.on_tool_end(make_mock_context(), make_mock_agent(), make_mock_tool("t"), "ok")
        await rh.on_tool_end(make_mock_context(), make_mock_agent(), make_mock_tool("t"), "ok")
    ...
    async def test_post_turn_skill_not_double_queued_when_both_hooks_active(self) -> None:
        ...
        await sh.on_llm_end(make_mock_context(), make_mock_agent(), make_mock_response())
        await rh.on_llm_end(make_mock_context(), make_mock_agent(), make_mock_response())
    ...
    async def test_pending_blocks_not_double_injected_when_both_hooks_drain(self) -> None:
        ...
        registry = SkillRegistry()
        skill = SimpleSkill("s", triggers_after_tools=["t"], content="unique-block")
        registry.register(skill)
        ...
        await asyncio.gather(
            sh.on_llm_start(make_mock_context(), make_mock_agent(), None, input_items),
            rh.on_llm_start(make_mock_context(), make_mock_agent(), None, input_items),
        )
...
class TestTriggersAfterToolsIgnoresContent:
    ...
    async def test_same_skill_queued_for_different_result_strings_from_same_tool(
        ...
        registry = SkillRegistry()
        skill = SimpleSkill("classifier", triggers_after_tools=["run_pipeline"])
        registry.register(skill)
        ...
        await hooks.on_tool_end(
            ctx, agent, make_mock_tool("run_pipeline"), '{"root_cause": "NEXTHOP_UNRESOLVABLE"}'
        )
        ...
        await hooks.on_tool_end(
            ctx, agent, make_mock_tool("run_pipeline"), '{"root_cause": "QUEUE_CONGESTION"}'
        )
    ...
    async def test_different_tool_name_does_not_queue_skill_regardless_of_result_content(
        ...
        registry = SkillRegistry()
        registry.register(SimpleSkill("my-skill", triggers_after_tools=["tool-a"]))
        ...
        await hooks.on_tool_end(
            ctx, agent, make_mock_tool("tool-b"), '{"root_cause": "NEXTHOP_UNRESOLVABLE"}'
        )
    ...
    async def test_all_skills_for_a_tool_name_are_queued_regardless_of_result(
        ...
        registry = SkillRegistry()
        registry.register(SimpleSkill("skill-one", triggers_after_tools=["pipeline"]))
        registry.register(SimpleSkill("skill-two", triggers_after_tools=["pipeline"]))
        registry.register(SimpleSkill("unrelated", triggers_after_tools=["other-tool"]))
        ...
        await hooks.on_tool_end(ctx, agent, make_mock_tool("pipeline"), "any result whatsoever")
