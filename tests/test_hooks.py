...
class _SimpleSkill(Skill):
    """Configurable skill used throughout these tests."""
...
async def _fire_end(hooks: SkillHooks | RunSkillHooks) -> None:
    """Call on_llm_end with minimal SDK arguments."""
...
def _injected_contents(items: list[Any]) -> list[str]:
    """Extract the content strings from all dict items in the list."""
...
class TestSkillHooksInjection:
    async def test_single_skill_prepends_block(self) -> None:
        hooks = SkillHooks([_SimpleSkill("s", content="skill content", always_on=True)])
    ...
    async def test_skill_block_appears_before_existing_items(self) -> None:
        hooks = SkillHooks([_SimpleSkill("s", content="prepended", always_on=True)])
    ...
    async def test_empty_input_items_receives_blocks(self) -> None:
        hooks = SkillHooks([_SimpleSkill("s", content="block", always_on=True)])
        items: list[Any] = []
    ...
    async def test_original_list_object_is_mutated_not_replaced(self) -> None:
        hooks = SkillHooks([_SimpleSkill("s", content="injected", always_on=True)])
        items: list[Any] = []
...
class TestSkillHooksOrdering:
    async def test_multiple_skills_inject_in_registration_order(self) -> None:
        hooks = SkillHooks(
            [
                _SimpleSkill("skill_first", content="first", always_on=True),
                _SimpleSkill("skill_second", content="second", always_on=True),
                _SimpleSkill("skill_third", content="third", always_on=True),
            ]
        )
        items: list[Any] = []
    ...
    async def test_skill_blocks_precede_original_items_after_multi_skill_injection(
        ...
        hooks = SkillHooks(
            [
                _SimpleSkill("skill_a", content="a", always_on=True),
                _SimpleSkill("skill_b", content="b", always_on=True),
            ]
        )
    ...
    async def test_multi_block_skill_followed_by_single_block_skill(self) -> None:
        hooks = SkillHooks(
            [
                ...
                _SimpleSkill("s", content="single", always_on=True),
            ]
        )
        items: list[Any] = []
...
class TestDisabledSkills:
    async def test_disabled_skill_is_not_injected(self) -> None:
        hooks = SkillHooks([_SimpleSkill("s", content="bad", enabled=False, always_on=True)])
        items: list[Any] = [{"role": "user", "content": "user message"}]
    ...
    async def test_enabled_and_disabled_skills_mixed(self) -> None:
        hooks = SkillHooks(
            [
                _SimpleSkill("on", content="enabled", always_on=True),
                _SimpleSkill("off", content="bad", enabled=False, always_on=True),
            ]
        )
        items: list[Any] = []
    ...
    async def test_all_disabled_leaves_items_unchanged(self) -> None:
        hooks = SkillHooks(
            [
                _SimpleSkill("off1", enabled=False, always_on=True),
                _SimpleSkill("off2", enabled=False, always_on=True),
            ]
        )
        items: list[Any] = [{"role": "user", "content": "original"}]
...
class TestMultiTurnInjection:
    async def test_skill_reinjects_after_on_llm_end_clears_guard(self) -> None:
        ...
        hooks = SkillHooks([_SimpleSkill("s", content="content", always_on=True)])
...
class TestSkillHooksWithRegistryNoRouter:
    ...
    async def test_disabled_skill_not_injected(self) -> None:
        registry = SkillRegistry()
        registry.register(_SimpleSkill("off", content="bad", enabled=False, always_on=True))
...
class TestManifestInjection:
    ...
    async def test_manifest_not_injected_without_registry(self) -> None:
        hooks = SkillHooks(skills=[_SimpleSkill("standalone", always_on=True)])
        items: list[Any] = []
...
class TestRunSkillHooks:
    async def test_injects_direct_skill(self) -> None:
        hooks = RunSkillHooks(skills=[_SimpleSkill("s", content="run content", always_on=True)])
        items: list[Any] = [{"role": "user", "content": "user message"}]
...
class TestInvokeSkillContentDelivery:
    ...
    async def test_invoke_skill_returns_string_not_prompt_blocks(self) -> None:
        ...
        registry = SkillRegistry()
        registry.register(_SimpleSkill("guidance", content="remediation steps"))
        tool = make_invoke_skill_tool(registry)
    ...
    async def test_invoke_skill_result_and_separate_tool_result_both_present_before_llm_call(
        ...
        registry = SkillRegistry()
        registry.register(
            _SimpleSkill("nexthop-unresolvable", content="NEXTHOP: check route table")
        )
        hooks = SkillHooks(registry=registry)
        tool = make_invoke_skill_tool(registry)
    ...
    async def test_unknown_skill_name_returns_descriptive_error_not_exception(self) -> None:
        ...
        registry = SkillRegistry()
        registry.register(_SimpleSkill("real-skill"))
        tool = make_invoke_skill_tool(registry)
...
class TestRouterAndInvokeSkillCoexist:
    ...
    async def test_router_injects_broad_skill_while_invoke_skill_fetches_narrow_skill(
        ...
        registry.register(_SimpleSkill("nexthop-unresolvable", content="NEXTHOP REMEDIATION"))
    ...
    async def test_invoke_skill_does_not_affect_router_selection_on_next_turn(self) -> None:
        ...
        registry = SkillRegistry(router=router)
        registry.register(_SimpleSkill("broad-skill"))
        registry.register(_SimpleSkill("narrow-skill"))
    ...
    async def test_router_selected_and_invoked_skill_both_appear_in_items(self) -> None:
        ...
        registry = SkillRegistry(router=router)
        registry.register(_SimpleSkill("topic-skill", content="TOPIC GUIDANCE"))
        registry.register(_SimpleSkill("usecase-skill", content="USECASE GUIDANCE"))
